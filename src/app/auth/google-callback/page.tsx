...
function GoogleCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (status === 'unauthenticated' || !session?.user) {
      // Not authenticated via Google, redirect to login
      toast({ title: 'Error', description: 'No se pudo autenticar con Google', variant: 'destructive' })
      router.push('/login')
      return
    }

    if (status === 'authenticated' && session.user && !processing) return
    ...
    async function processGoogleLogin() {
      try {
        ...
        toast({
          title: 'Error',
          description: err instanceof Error ? err.message : 'Error al autenticar con Google',
          variant: 'destructive',
        })
        router.push('/login')
      }
