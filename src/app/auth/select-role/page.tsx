  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-neutral-50 to-neutral-100 p-4">
      ...
      <button
        ...
        className="absolute top-4 left-4 flex items-center gap-1.5 text-sm text-neutral-500 hover:text-neutral-700 transition-colors"
      >
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-3">
          <div className="mx-auto w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 overflow-hidden flex items-center justify-center ring-4 ring-white shadow-lg">
            {userInfo.picture ? (
          ...
          <div>
            <h1 className="text-xl font-bold text-neutral-900">Hola, {userInfo.name}!</h1>
            <p className="text-sm text-neutral-500">{userInfo.email}</p>
          </div>
          <p className="text-neutral-600">¿Cómo deseas usar la plataforma?</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-start gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                selectedRole === 'admin' ? 'bg-white/20' : 'bg-neutral-100'
              }`}>
                <Store className={`w-5 h-5 ${selectedRole === 'admin' ? 'text-white' : 'text-neutral-600'}`} />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className={`font-semibold ${selectedRole === 'admin' ? 'text-white' : 'text-neutral-900'}`}>
                  Soy Vendedor
                </h3>
                <p className={`text-sm ${selectedRole === 'admin' ? 'text-white/70' : 'text-neutral-500'}`}>
          ...
          {selectedRole === 'admin' && (
            ...
            >
              ...
              <input
                ...
                className="w-full h-11 px-4 rounded-xl border border-neutral-200 text-sm focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent"
              />
          ...
          >
            <div className="flex items-start gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                selectedRole === 'customer' ? 'bg-white/20' : 'bg-neutral-100'
              }`}>
                <User className={`w-5 h-5 ${selectedRole === 'customer' ? 'text-white' : 'text-neutral-600'}`} />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className={`font-semibold ${selectedRole === 'customer' ? 'text-white' : 'text-neutral-900'}`}>
                  Soy Cliente
                </h3>
                <p className={`text-sm ${selectedRole === 'customer' ? 'text-white/70' : 'text-neutral-500'}`}>
        ...
        <Button
          ...
          className="w-full h-12 rounded-xl font-semibold text-sm gap-2 bg-neutral-900 hover:bg-neutral-800 text-white disabled:bg-neutral-200 disabled:text-neutral-400"
        >
          {loading ? (
            <>
