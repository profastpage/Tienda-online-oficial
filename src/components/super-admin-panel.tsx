...
export default function SuperAdminPanel() {
  ...
  // ═══ Auth — ALWAYS validate with server ═══
  useEffect(() => {
    (async () => {
      try {
        // Always call /api/auth/me to validate session (cookie + token)
        const meRes = await fetch('/api/auth/me', { credentials: 'include' })
        if (!meRes.ok) { router.push('/login'); return }
        const userData = await meRes.json()
        if (userData?.role !== 'super-admin') { router.push('/login'); return }
        // Valid super-admin — store fresh data
  ...
  const apiCall = useCallback(async (body: Record<string, any>) => {
    ...
    if (res.status === 401) { router.push('/login'); return { error: 'Sesion expirada' } }
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        // Session expired — clear stale data and redirect
